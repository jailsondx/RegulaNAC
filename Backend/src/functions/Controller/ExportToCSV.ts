...
/*IMPORT INTERFACES*/
import { ReportsPacienteCSV, ReportsGerenciaCSV } from '../../Interfaces/Relatorios';
...
async function exportarRelatorioCSVPaciente(dados:ReportsPacienteCSV[]) {
    try {
        if (dados.length === 0) {
...
async function exportarRelatorioCSVGerencial(dados:ReportsGerenciaCSV[]) {
    try {
        if (dados.length === 0) {
            throw new Error('Nenhum dado disponível para exportação.');
        }
